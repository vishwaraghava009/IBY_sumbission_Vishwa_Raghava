#cherry-picked from https://github.com/huyvnphan/PyTorch_CIFAR10/blob/master/cifar10_models/resnet.py
...
def _resnet(arch, block, layers, pretrained, progress, device, **kwargs):
    ...
    if pretrained:
        models_dir = os.path.dirname(os.path.abspath(__file__))  # Directory of resnet.py
        if not weights_downloaded:
            download_weights(models_dir)
            weights_downloaded = True

        state_dict_path = os.path.join(models_dir, "cifar10_models/state_dicts", arch + ".pt")
...
def download_weights(models_dir):
    state_dicts_dir = os.path.join(models_dir, "cifar10_models")
    ...
    zip_path = os.path.join(models_dir, "state_dicts.zip")
    ...
    with zipfile.ZipFile(zip_path, "r") as zip_ref:
        zip_ref.extractall(state_dicts_dir)
