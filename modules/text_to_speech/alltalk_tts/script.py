...
enable_phoneme_extraction = params.get("enable_phoneme_extraction", False)
json_output_path = params.get("json_output_path", params["output_folder_wav"])
...
def extract_phonemes_wav2vec2(audio_path):
    try:
        logging.info(f"Starting phoneme extraction for {audio_path}")
        ...
        phonemes_with_timestamps = [(item["text"], item["timestamp"][0], item["timestamp"][1]) for item in result["chunks"]]
        logging.info(f"Phoneme extraction completed for {audio_path}")
        return phonemes_with_timestamps
    except Exception as e:
        logging.error(f"Error extracting phonemes: {e}")
        return []
...
def phonemes_to_morph_indices(phonemes):
    ...
    return [{
        ...
        "end_time": end_time,
        "morph": phoneme_to_morph_map.get(phoneme, None)
    } for phoneme, start_time, end_time in phonemes]

def save_json(data, output_filename):
    json_filename = Path(output_filename).stem + ".json"
    json_output_fullpath = Path(json_output_path) / json_filename
    with open(json_output_fullpath, 'w') as json_file:
        ...
        try:
            if params["narrator_enabled"]:
                ...
                for part_type, part in processed_parts:
                    ...
                    if params["enable_phoneme_extraction"]:
                        # Extract phonemes and map to morph indices
                        print(f"[DEBUG] Extracting phonemes for: {audio_path}")
                        phonemes_with_timestamps = extract_phonemes_wav2vec2(audio_path)
                        morph_indices = phonemes_to_morph_indices(phonemes_with_timestamps)
                        print(f"[DEBUG] Morph indices: {morph_indices}")

                        # Save JSON with morph indices and timestamps
                        print(f"[DEBUG] Saving JSON for: {output_filename}")
                        save_json(morph_indices, output_filename)
                ...
                output_file_str = output_file.as_posix()
                output_file = get_output_filename(state)
                print(f"[DEBUG] Output filename: {output_file}")
                generate_response = send_generate_request(
                    cleaned_part,
                    params["voice"],
                    language_code,
                ...
                if params["enable_phoneme_extraction"]:
                    ...
                    # Save JSON with morph indices and timestamps
                    print(f"[DEBUG] Saving JSON for: {output_file}")
                    save_json(morph_indices, output_file)
